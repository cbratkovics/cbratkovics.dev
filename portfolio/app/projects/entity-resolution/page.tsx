        ...
        <div className="case-study-prose">
          ...
          <Section title="What the data said">
            <div className="space-y-6">
              <FindingBlock title="The rules baseline was the better single method" why={<p>A learned model is not automatically an upgrade. Here its measurable contribution was calibrated confidence and a review queue about a quarter the size, bought with lower recall, at different tier thresholds.</p>} recommendation={<ul><li>Keep the rules baseline as the reference method.</li><li>Switch to the classifier only if reviewer hours are the binding cost and you accept the recall you give up.</li><li>Do not compare the queues as if recall were equal.</li></ul>}><ul><li>On the test fold at auto-accept, rules scored precision 0.9946, recall 0.931, F1 0.962.</li><li>The calibrated classifier scored precision 0.9993, recall 0.850, F1 0.919.</li><li>Review queues: 25,076 records for rules, 6,016 for the classifier.</li></ul></FindingBlock>
              <FindingBlock title="Coverage is not accuracy" why={<p>In production matching, this is exactly the population a high match rate is built from. Without labels, a rising coverage number tells you nothing about false attribution.</p>} recommendation={<p>Report verified and unverified accepts as separate numbers, and treat the unverified share as review backlog, not as wins.</p>}><p>The rules method auto-accepted 4,501 MusicBrainz records that have no Discogs link. Those are unverified, and they appear in coverage counts only, never in precision, recall, or F1.</p></FindingBlock>
            </div>
          </Section>
          ...
          <Section title="How it was built"><p>The project was AI-assisted, using Claude Code under a written brief with phase-gated review, which means I approved or amended every phase. The brief and its decision records (ADR 0001 to 0005) are in the repository.</p><p>Independent project on CC0 data; MIT licence; no employer code, data, or business rules.</p><p><strong>Stack:</strong> Python, SQL, scikit-learn, dbt-core, DuckDB, uv, pytest, GitHub Actions, GitHub Pages.</p><div className="mt-7"><EvidenceLinks links={links} /></div></Section>
        </div>
      </article>
    </main>
  </>;
